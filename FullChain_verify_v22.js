...
(async () => {
  ...
  await page.goto('file://' + require('path').resolve('TMF_FullChain_Simulator_v22.html'));
  ...
  // 15. Siegert term: lambda and flue-gas temperature must now move efficiency
  R.siegert = await page.evaluate(() => {
    const save = { l: P.lambda, t: P.T_air };
    const at = l => { P.lambda = l; _pipeParamHash = ''; const c = runPipeline().comb;
                      return { eta: +c.eta_c.toFixed(4), O2: +c.O2_pct.toFixed(2),
                               qs: +(c.q_stack * 100).toFixed(2), Tfg: +c.T_fg.toFixed(0) }; };
    const a = at(1.05), b = at(1.20), c2 = at(1.80), d2 = at(2.50);
    P.lambda = save.l; _pipeParamHash = ''; runPipeline();
    return { at1_05: a, at1_20: b, at1_80: c2, at2_50: d2,
             lambda_now_costs_efficiency: a.eta > b.eta && b.eta > c2.eta && c2.eta > d2.eta };
  });

  // 16. Exergy cascade must account for the fuel without going negative
  R.exergyCascade = await page.evaluate(() => {
    _pipeParamHash = '';
    const r = runPipeline();
    const c = calcExergyCascade(r.comb, r.boil, r.turb, r.gen);
    const sum = c.X_comb_lost + c.X_boiler + c.X_stack + c.X_turb + c.X_cond + c.X_gen + c.P_elec;
    return {
      fuel_exergy_kW: +c.X_fuel.toFixed(0),
      combustion_destroyed_kW: +c.X_comb_lost.toFixed(0),
      combustion_pct: +(c.X_comb_lost / c.X_fuel * 100).toFixed(1),
      boiler_dT_kW: +c.X_boiler.toFixed(0),
      P_elec_kW: +c.P_elec.toFixed(1),
      second_law_efficiency_pct: +(c.eta_ex * 100).toFixed(1),
      accounted_kW: +sum.toFixed(0),
      closure_error_pct: +((sum - c.X_fuel) / c.X_fuel * 100).toFixed(1),
      all_terms_non_negative: [c.X_comb_lost, c.X_boiler, c.X_stack, c.X_turb, c.X_cond, c.X_gen].every(v => v >= 0),
      combustion_exceeds_output: c.X_comb_lost > c.P_elec
    };
  });

  // 17. Corrosion-limited recovery window and the drying loop
  R.recovery = await page.evaluate(() => {
    const save = P.fuel, out = {};
    for (const f of ['wood_chip', 'straw', 'pellet']) {
      P.fuel = f; _pipeParamHash = '';
      const r = runPipeline(), lim = calcStackLimit(r.comb);
      out[f] = { Cl_pct: FUELS[f].Cl_pct, T_dew: +lim.T_dew.toFixed(1), T_min: +lim.T_min.toFixed(0),
                 recoverable_kW: +lim.Q_avail.toFixed(0), blocked_kW: +lim.blocked.toFixed(0) };
    }
    P.fuel = save; _pipeParamHash = '';
    const r2 = runPipeline();
    const dl = calcDryingLoop(r2.comb, r2.gen);
    return { byFuel: out, dryingHeatAvailable_kW: +dl.Q_avail.toFixed(0),
             dryingSteps: dl.rows.slice(1).map(x => `to ${x.mc}% MC: needs ${x.Q_dry.toFixed(1)} kW, gives +${x.gain.toFixed(1)} kW, feasible=${x.feasible}`),
             paramsRestored: P.fuel === save };
  });


  // 18. Tab groups [v2.1] — every tab must belong to exactly one group, and
  //     jumping to a hidden tab must bring its group along.
  R.tabGroups = await page.evaluate(() => {
    const all = TAB_GROUPS.flatMap(g => g.tabs).sort((a, b) => a - b);
    const dupes = all.filter((v, i) => all.indexOf(v) !== i);
    switchTab(9);                       // DataLog lives in LIVE DATA, not CHAIN
    const jumped = { group: activeGroup, tabVisible: document.querySelectorAll('.tabs .tab')[9].style.display !== 'none',
                     panelActive: document.getElementById('panel9').classList.contains('active') };
    setGroup(0);
    return { totalTabsCovered: all.length, duplicates: dupes,
             coversAll13: all.length === 13 && all[0] === 0 && all[12] === 12,
             jumpBringsGroup: jumped };
  });

  // 19. Treatment train — order must change the outcome, and a scrubber first
  //     must starve a downstream economiser.
  R.treatmentTrain = await page.evaluate(() => {
    const save = { a: P.tr1, b: P.tr2, c: P.tr3 };
    const run = (a, b, c) => { P.tr1 = a; P.tr2 = b; P.tr3 = c; _pipeParamHash = '';
      const t = runTreatmentTrain(runPipeline().comb);
      return { Q: +t.Q_rec.toFixed(1), hcl: +(t.captured * 100).toFixed(0), Tout: +t.T_out.toFixed(0),
               lime: +t.kgh_Ca.toFixed(2) }; };
    const econOnly   = run('econ', 'none', 'none');
    const econDry    = run('econ', 'dry', 'none');
    const econCond   = run('econ', 'cond', 'none');
    const wetThenEcon= run('wet', 'econ', 'none');
    const condThenEcon=run('cond', 'econ', 'none');
    P.tr1 = save.a; P.tr2 = save.b; P.tr3 = save.c; _pipeParamHash = ''; runPipeline();
    return { econOnly, econDry, econCond, wetThenEcon, condThenEcon,
             order_matters: econCond.Q !== wetThenEcon.Q,
             scrubber_first_starves_economiser: wetThenEcon.Q < econOnly.Q,
             condensing_recovers_most: econCond.Q > econOnly.Q };
  });

  R.consoleErrors = errors;

  for (const [tab, name] of [[1, 'boiler'], [2, 'turbine'], [3, 'generator'], [5, 'anim'], [7, 'otdd'], [10, 'sensitivity'], [11, 'trajectory'], [12, 'flue']]) {
