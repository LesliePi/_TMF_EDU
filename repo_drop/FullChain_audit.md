...
- Artefact: `TMF_FullChain_Simulator_v18.html` — single file, no build step, no network
...
**1. The σ_μ layer does not feed the power chain. ∂P_elec/∂σ = 0, exactly.**

This is the big one. `η_TMF` is computed, displayed, logged and charted — but
shaft work is built from the real enthalpies alone:

```
W_mech = m_steam · w_net,   w_net = (h₁ − h₂) − w_pump
```

`η_TMF` never multiplies anything. So dragging σ_T, σ_ρ or σ_v moves a number on
screen and changes nothing about the electricity produced. The Sensitivity tab
(tab 11) measures this directly and reports it in red.

Either `η_TMF` should scale the shaft work — which would make the framework's
central claim load-bearing and testable end-to-end — or it should be relabelled
as a diagnostic index rather than an efficiency. It is currently neither, and
that is an unresolved modelling decision, not an oversight.
...
These are machine-checked in `FullChain_verify_v18.js` (headless Chromium). Re-run it to
...
0. **The disconnect in §2.6 item 1.** Before anything else: decide whether
   `η_TMF` should scale the shaft work. As it stands the framework's central
   quantity has no effect on the framework's central output. Everything below is
   secondary to this.
...
node FullChain_verify_v18.js         # invariants + sweeps + screenshots
```

`FullChain_verify_v18.js` needs Playwright and a Chromium binary. It loads the HTML from a
